{{--  
    This query is written directly in the Blade template because the menu is present on every page, 
    and it may not be within the scope of individual controllers. 
    By placing the query here, we ensure that the menu data is available wherever needed.

    Eager loading is used to optimize the query by loading subCategories and childCategories 
    in a single call, reducing the number of queries to the database. This prevents the N+1 problem 
                            ...
                            @if (count($categoryItem->subCategories) > 0)
                                <div id="flush-collapseThreew-{{ $loop->index }}" class="accordion-collapse collapse"
                                    ...
                                    <div class="accordion-body">
                                        <ul>
                                            @foreach ($categoryItem->subCategories as $subCategoriesItem)
                                                <li><a href="#">{{ $subCategoriesItem->name }}</a></li>
                                            @endforeach
                                        </ul>
                                    </div>
                                </div>
